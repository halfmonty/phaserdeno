...
With this method, we load the music that will be played during the story. The next method loads audio files (just the type) and then we have a method to play them.
  */
	...
	playAudio(key:string) {
		if (this.audios) this.audios[key].play();
	}
...
This is a text intro that is shown before the videos. It is typed and then removed.
  */
	showIntro() {
		const text1 = this.utils.typeText(
			' IN 1968 YURI GAGARIN DIED\nDURING A ROUTINE FLIGHT',
			'computer',
			this.center_width,
			this.center_height,
		);
		let text2: (Phaser.GameObjects.BitmapText | Phaser.GameObjects.Rectangle)[];
		...
		this.time.delayedCall(7000, () => this.playMusic(), undefined, this);
		this.time.delayedCall(
			10000,
			() => {
  ...
  */
	aGameBy() {
		const text1 = this.utils.typeText(' A GAME BY\nPELLO', 'computer', 1250, 10);
		let text2: (Phaser.GameObjects.BitmapText | Phaser.GameObjects.Rectangle)[];
		const pelloLogo = this.add
			.image(990, 120, 'pello_logo_old')
			.setScale(0.2)
			.setOrigin(0.5);
		const video = this.add.video(400, 300, 'video0');
  ...
  */
	tools() {
		...
		let text2: (Phaser.GameObjects.BitmapText | Phaser.GameObjects.Rectangle)[];
		const video = this.add.video(this.center_width, 500, 'video1').setOrigin(0.5);
  ...
  */
	otherTools() {
		...
		let text2: (Phaser.GameObjects.BitmapText | Phaser.GameObjects.Rectangle)[];
		const video = this.add.video(this.center_width, 100, 'video2').setOrigin(0.5);
...
Finally, another video and more credits for the music.
  */
	lastVideo() {
		...
		let text2: (Phaser.GameObjects.BitmapText | Phaser.GameObjects.Rectangle)[];
		const video = this.add.video(this.center_width, 400, 'video3').setOrigin(0.5);
